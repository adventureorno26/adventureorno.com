...
| 7 | **Add / remove / block another user** — a directory, a public profile with a handle, and a connection that can be added, **removed** and **blocked**. Blocking is bidirectional and enforced in RLS, never only in the UI. The word is **add**, not friend (§0.2) | 🟡 **DATABASE IS LIVE, SCREENS IN FLIGHT.** `0283`/`0284`/`0285`/`0286` are applied to production (PR #188): handles, `find_profiles()`, `public_profile()`, `connection_adds`, `connection_follows`, `connection_blocks`, `is_blocked_between()`. Blocking is **bidirectional and enforced in RLS**, proven with one role switch per transaction — a first test that used a bare `reset role` gave a false pass, because that returns to `postgres` and a superuser bypasses RLS entirely. Two bugs were found and fixed on the way: a `handle NOT NULL` that made backups **unrestorable** (`restore-data.sh` runs under `session_replication_role = replica`, which disables the assigning trigger — fixed with a column DEFAULT in `0285`), and `profiles_owner_write` being `FOR ALL`, which **includes SELECT**, so an owner could read straight through a block (`0286`). The directory and public-profile SCREENS are not built yet. |
