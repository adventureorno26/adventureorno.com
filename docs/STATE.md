...
### THE CONTROL IS TWO BUTTONS AND A PICKER — built 2026-08-30

Measured on production the same day, the map's bottom control read `[My Stats]  [Josh]` —
half of this model and a pill of the old one, which is worse than either alone. `My Stats`
is a scope; `Josh` is a person; the row offered two different kinds of answer under one
control and neither of them was Our Stats. Erica saw it and objected.

What is live now, on all three screens:

* **`My Stats` / `Our Stats`, and no third button.** Everything still opens on My Stats.
* **Pressing `Our Stats` opens a picker** — a sheet listing the people you have recorded,
  with one or more tickable. Nothing changes until its own button is pressed, and that
  button is **disabled while nobody is picked**: an empty pick would be My Stats wearing
  the other word, which is the defect `0280` exists to end.
* **Pressing `Our Stats` again re-opens the picker**, because `Our Stats` with Josh and
  `Our Stats` with Josh and Maya are different questions under one label — who is in it is
  the whole meaning of the number.
* **The map names who** in the filter chips over the pins (`Josh ×`), which is where a
  narrowing is already announced and cleared. Settings and Insights name them in the
  scope sentence beside the numbers. Neither is a pill you press to select a person.
* **The third scope keeps its home**: `/people/:personId`. Its `All of them / Any of them`
  switch is gone with the rest of the operator, so "also with" there is an intersection
  too, and `fetchMemoriesWithPeople` no longer accepts `'any'` at the type level.

Guarded by `app/src/lib/participants.test.ts`, which names each of the eight scope surfaces
individually and fails on any retired word — the shape that was missing when `/bucket` kept
"Both" for a year under a guard that only ever read `Settings.tsx`.

### MILES COME FROM `mileage_by_person_for_people` — corrected 2026-08-30

**This supersedes the "2,108 miles" recorded against item 5 below.** The map's miles pill
and the Insights Miles tile both read `wander_stats_for_people`, whose miles clause requires
`a.place_id is not null` — so an outing never attached to a place is missing from the total.
Measured on production as Erica: the pill said **2108.5** while the drill-down list directly
underneath it, which reads `mileage_by_person_for_people`, summed to **2135.6**. Two numbers
for one question inside a single control.

**2135.6 is the approved figure** in the table above, and the readers agree on Our Stats
(481.6 either way) because those outings all have places. Both screens now read the mileage
function. `wander_stats_for_people` still supplies the PLACES count, which is 132 / 55 and
matches `place_ids_for_people` exactly.
...
**CLOSED 2026-08-30 by migration 0287, which is applied to production.** The paragraph that
stood here said the opposite and is kept because it was true when written:
...
> account and RLS cannot save them… **The honest fix is in the database:** teach both
...
That fix landed. `0287_a_public_profile_that_was_never_once_read.sql` re-creates
`public_profile()` with an explicit `if public.is_blocked_between(pr.id, auth.uid()) then`
guard, and `find_profiles()` with `and not public.is_blocked_between(p.id, auth.uid())` in
its where clause. The migration is recorded in production's ledger (`npm run check:ledger`,
all 288 recorded), so the database — not the client — now refuses a blocked reader.

**WHAT IS PROVED AND WHAT IS NOT, stated separately on purpose.** What is proved is that the
migration is applied and that both function bodies carry the guard. What is **not** proved is
the runtime behaviour: demonstrating that a blocked account calling PostgREST directly is
actually refused needs two real accounts with a block between them, and that test has not been
run. Until it is, this is "the fix is deployed", not "the bypass is measured shut".
...
2b. ~~**An accepted tag becomes the accepter's own record**, and removal retracts rather
   than deletes.~~ ✅ **DONE 2026-08-31 — `0294` and `0299`, both applied to production.**
   `0294` made removal a retraction; `0299` makes acceptance materialise an activity row
   owned by the accepter, in their space, on the original's `shared_group_id` — so their
   untag, block and deletion cannot reach it, and the 15-mile run still counts once.
   **OUTINGS ONLY**, per `0292`'s finding that visits and places have no dedupe key: an
   accepted VISIT tag is still hostage, and that is the biggest thing left under
   §AN ACCEPTED TAG IS MINE. *(Original: without this, everything a person accepts stays
...
The pills are last because they are the only easy part. **They are done, and so is 2b as of
2026-08-31. Item 3 — cross-account tagging — is the only one of the five still open, and it
is what gives `0299` a live case: until a tag can cross an account, the rule that survives
the tagger never fires.**

---

## WHAT IS OUTSTANDING AFTER 2026-08-31 — one list, so none of it lives only in a header

Seven migrations went to production on 2026-08-31 (`0293`–`0299`). Each was rehearsed against
production inside a transaction forced to abort, with the **rollback proven** before the real
apply, and every capability figure for all three accounts was byte-identical before and after
all seven. What follows is what those seven did NOT close. Each names where the evidence is,
because a sentence inside a merged migration header is not where an outstanding item survives.
...
| 4b | **The scope model on the live UI** — `My Stats` / `Our Stats` on the map, Settings ▸ Stats and Insights, with the per-person pill removed and a picker for who Our Stats is about | ✅ **BUILT 2026-08-30, numbers proven against production** — see §0.2 above for the control's shape. The three scopes were measured as Erica through the real readers before a line was written: **My Stats 132 places · 2135.6 mi**, **Our Stats (Erica + Josh) 55 places · 481.6 mi**, **Josh's own 61 places · 1053.7 mi** — every figure §0.2 approved, to the decimal, from `place_ids_for_people` and `mileage_by_person_for_people` on `public.people` ids. **No migration; nothing in the database changed.** One defect found on the way and fixed: the miles pill and the list underneath it were reading two different functions and disagreeing by 27 miles — recorded above, and it supersedes the "2,108 miles" in item 5. One defect found in the browser rather than in a test: at 390×844 the picker's own button sat under the floating nav and the click landed on the Settings tab, because `position: fixed` on `.map-root` traps everything inside it in a stacking context below `nav.primary-nav` — `--pnav-clearance` again, the same answer as every other sheet. **What was NOT done, deliberately:** §0.2's retired list also covers the *attribution* vocabulary (`Together · Just me · Just <name>`) that item 4 above had just unified across seven surfaces, and §0.2 supplies no replacement for it — "who was on this visit" is a different question from "whose numbers are these", and inventing words for it is a decision, not an implementation. Those surfaces are untouched and unguarded here; the guard names the eight SCOPE surfaces. The DB comment on `place_ids_for_people` still says "Anyone"/"Together" too, and changing it needs a migration. |
| 4c | **The people picker** — the attribution vocabulary item 4b deliberately left alone | ✅ **BUILT 2026-08-30, merged (#189).** §0.2 retired `Together · Just me · Just <name> · Both · All` as SCOPE words and supplied no replacement for the ATTRIBUTION question; 4b said so and left those seven surfaces untouched, because *"who was on this visit" is a different question from "whose numbers are these"*. Erica then decided: *"yes, people picker."* You tag whoever was there **by name**; nobody ticked means it was just you. One `WhoPicker` on all seven surfaces, rendering the **same `.scope-picker*` sheet PR #187 already proved on a phone** against `--pnav-clearance`, so the two controls cannot drift apart. They are deliberately NOT one component — a scope is *whose numbers are these* and this is *who was on this card*, and a component taking a flag to be either is how the two questions started looking interchangeable. **Nothing is applied until save**, so ticking a name cannot fire an RPC per tap and Cancel genuinely cancels. **Capacity is not decoration:** `set_place_solo` and `set_activity_solo` take ONE profile id and only the visit path takes a list, so those surfaces pass `capacity="one"` and the sheet **says so** — the alternative, looking multi-select and keeping the first name, is a control that lies about what it saved. `whoSingle()` refuses to collapse. 330 unit tests green, including a guard per surface. ✅ **LIVE-VERIFIED 2026-08-30** on production (`version.json` = `afb4225`), signed in as the owner, at `/places/edit`: **zero retired words anywhere on the page**, and the rows read **"Josh"** and **"You and Josh"** by name where they used to say *Just Josh* and *Together*. Both variants of the sheet open and are visibly the SAME control asking different questions — the filter says *"Whose places? Show only the places everyone you tick has been to. Tick nobody to show them all."*, the attribution says *"Who was there? Tick everyone who was there. Nobody ticked means it was just you."* with Josh already ticked. **Cancel genuinely cancels** — the sheet closed and the row still read the same thing, nothing written. |
| 5 | **Reconcile the disagreeing numbers** (finding 3) | ✅ **LIVE-VERIFIED 2026-08-30** — Trips reads **43 on Insights and 43 on Settings**, measured on production as the owner. It was **17 vs 56**. Every other figure matches the predicted post-fix value: 132 places, 2,108 miles, 75 trails taken, 13 states, 38 summits, 182,735 ft. **The test bot reads 0 everywhere and that is correct** — it holds no participations, and the default scope is now My Stats rather than everybody, so it was the wrong account to verify with. Originally — migration `0280`, §6e.** The 17-versus-56 is one wiring fault: `null` means the OPPOSITE thing in the two generations of reader, and Settings called the old one while Insights called the new one. Given the SAME scope the two agree exactly (`trips_list(erica)` = `trips_list_for_people([erica])` = 43), which is what makes it wiring and not arithmetic. All three stats screens now share one scope control and open on **My Stats**; the fourteen `p_profile` wrappers are deleted from the app and a guard fails the build if a screen calls one of those RPCs again. **The place counts were three different questions and are documented as such rather than forced together** — 132 places you have BEEN, 151 saved place RECORDS, 168 rows a bulk editor can edit. |
| 6 | **Needs Attention** (finding 4) — filtered destinations per tile, and real error feedback when an RPC fails | ✅ **LIVE-VERIFIED 2026-08-30** — the six repair tiles now have **six distinct destinations** (`?needs=unnamed`, `?needs=untagged`, `?needs=undated`, `?needs=photo-dates`, `/duplicates`, `#cards`); the check reports **duplicate destinations: NONE**, against four identical links before. The silent failures were never a missing catch — every handler existed but was guarded by `e instanceof Error`, which is **false for what Supabase throws** (`PostgrestError` is a plain object, an interface not a class), so nine call sites caught the error and threw the reason away. `lib/whyItFailed.ts` now reads it and the message sits on the card. A **fifth dead tile** nobody had reported — `<Link to="#cards">` pushing a route and never scrolling — was found and fixed. — PR #181. |untagged\|undated\|photo-dates` narrows the 168-row table, and the place predicates are **exported from `fetchAttention` itself** (`lib/data`: `repairablePlaces`, `placeNeeds`) so the number on the tile and the rows at the other end cannot disagree the way 17-Trips and 56-Trips did. "Open map" is gone: `activities.place_id` has no client write path, so the outings it counted are **listed on the queue itself** with the day each happened, saying plainly that a place gets attached by answering the card below — showing the rows rather than sending her somewhere that cannot do the work. **A FIFTH DEAD TILE nobody had reported:** "Review them" was a `<Link to="#cards">`, and react-router pushes a location for a fragment WITHOUT scrolling — so on a page this long it did nothing visible either. Plain `<a href>` now, plus `scroll-margin-top` so the jump clears the back-bar. **On the silent RPC:** the cause was not a missing catch — every catch was there and called `showSnack`. It was `e instanceof Error`, which is **FALSE for what Supabase actually throws** (PostgrestError is a plain `{message, details, hint, code}` object, an interface not a class), so every failure in the app printed the generic fallback and threw the real reason away. New pure `lib/whyItFailed.ts` (companion to `saveOutcome.ts`, 14 unit tests) reads the reason, distinguishes "the server refused this" from "the server was never reached" from "you are offline" — different words because they need different actions, and only one of them can have changed anything — and the message now sits **on the card**, not only in a toast that outlives the evidence by six seconds. `Attention.suggestedTrips` (computed and returned since the trips table was retired in 0137, with no row reading it) is deleted. |
| 7 | **Add / remove / block another user** — a directory, a public profile with a handle, and a connection that can be added, **removed** and **blocked**. Blocking is bidirectional and enforced in RLS, never only in the UI. The word is **add**, not friend (§0.2) | ✅ **BUILT AND LIVE — corrected 2026-08-30 from *queued*, which it had not been for two days.** MEASURED ON PRODUCTION, signed in as the test bot, not read off the code: **`/people`** renders `People` · `Your people`, a search box placeheld *handle or name*, and *"Nobody yet. Search above, then add somebody — an add is mutual, so they decide too."* **`/settings/account/invites`** renders `Invite codes` · `Make a code` · `Your codes`, with *"Someone can only join with a code from you. Each code lets in one person and then stops working"* and expiry choices 1/7/14/30/90 days. **`/profile/<unknown handle>`** answers *"No page here — there is no public profile at that handle. It may not exist, or it may not be public — this app will not tell you which"*, which is the one-answer rule §CONNECTING TO SOMEONE asks for. **Settings ▸ Account** carries `Find people` and `Invite codes`; the nav is still exactly `Map · Add · Insights · Settings`, so this is not a fifth destination. The word is **add** — `/people` contains no *friend* and none of §0.2's seven retired words. **THE DEPENDENCY IN THIS ROW WAS BACKWARDS:** it said *"needs item 9 underneath it"*, and item 9 landed first while this shipped anyway. **WHAT IS NOT DONE, and it is the half that makes the screens useful:** 0283 defaults `profile_visibility` to `private` and all three `public_*` switches to `false`, and there is no UI to set a handle, an avatar, a bio or any of those switches — so `find_profiles()` and `public_profile()` currently return nothing for every account. The screens are real and empty. **Four live checks were added** to `erica-asked-for.spec.ts` on 2026-08-30 — it had thirty and not one mentioned people, profiles, handles, blocking or invites, which is why this row could sit at *queued* with nothing red. `app/src/lib/liveCheckCoverage.test.ts` now fails the build if a route ships uncovered. **Two bugs were found and fixed on the way and are worth keeping:** a `handle NOT NULL` that made backups **unrestorable** (`restore-data.sh` runs under `session_replication_role = replica`, which disables the assigning trigger — fixed with a column DEFAULT in `0285`), and `profiles_owner_write` being `FOR ALL`, which **includes SELECT**, so an owner could read straight through a block (`0286`). Blocking was proven with one role switch per transaction — a first test using a bare `reset role` gave a false pass, because that returns to `postgres` and a superuser bypasses RLS entirely. |
| 7b | **Cross-account tagging with acceptance** (findings 8, 9, 10) — a real user lookup, not pills; the tagged person must accept. Requires reconciling the people/profile seam: the read side keys on **people**, the write side on **profiles**, and `set_visit_solo` takes a single profile id | **THE ONLY ONE OF THE FIVE STILL OPEN, and everything else now waits on it.** `2b` (`0294`+`0299`) built the floor it stands on: the first tag that ever crosses an account lands on a rule that already survives the tagger. Its own blockers are unchanged and measured — findings 8, 9 and 10 above. |
| 8 | **The three SECURITY DEFINER views** | ✅ **DONE — LANDED IN `0289`, verified 2026-08-31.** `0289:768-830` recreates all three with the boundary written into each `WHERE` (`is_member(s.space_id)`, and `is_member(a.space_id)` for `activity_provenance`, correctly keyed on the activity rather than a subject), then sets `security_invoker = false` explicitly so the definer property is a decision rather than a default nobody chose. Checked against the live schema rather than the tick. *Originally:* **DECIDED 2026-08-30, FOLDED INTO ITEM 9.** Not flipped to `security_invoker` — that would make the app show a visit without showing who was on it (a third account can read 557 visits and would see 0 participants). The bypass is closed instead by writing each view's intended visibility explicitly, against the space boundary, and it **lands with the partition**. See the decision table above. **Still an absolute precondition for anyone else holding an account.** |
| 9 | **Spaces, friends, public profiles** (Phase 3b) — the gate on everything social. One indivisible migration | ✅ **DATABASE COMPLETE 2026-08-31 — reads AND writes.** `0293` closed the write half: 46 before-row triggers, one guard, and the reproduced exploit (Josh rewriting a visit in Erica's space) now refused with `42501`. `0295` gave caller-less writers a space; `0297` gave the fog one row per space; `0298` stopped anybody ever joining somebody else's. **Still not done:** no client surface, and correct behaviour for a second real account is still unproven — it needs two real accounts. *Originally:* **THE DATABASE HALF IS APPLIED TO PRODUCTION — corrected 2026-08-30 from *queued*.** `0289_a_space_is_the_boundary_and_it_says_so.sql` and `0290_the_readers_say_which_space_they_are_reading.sql` are both live. **Verified two independent ways against production, not from the code:** (a) `npm run check:ledger` reads `supabase_migrations.schema_migrations` on the live project and reports all 288 migrations recorded, 0289 and 0290 among them; (b) `npm run gen:types`, which regenerates `app/src/lib/database.types.ts` FROM the live schema, produces a **zero diff** against the committed file — and that file carries `spaces`, `space_memberships` and a `space_id` on ~70 tables. **WHY THIS ROW WAS WRONG FOR TWO DAYS:** both migration files opened with *"DRAFT — REHEARSED, NOT APPLIED. Nothing in this file has been run against production"*, and 0289 additionally called itself `0287`, which is a different applied migration. Both headers are corrected (comments only; no SQL touched) and `scripts/check-draft-migrations.mjs` now fails when a header's not-applied claim disagrees with the ledger, or when a file calls itself by another number. **WHAT IS NOT DONE:** there is no client surface at all — `space_id`/`in_space_` appear in exactly one file in `app/src`, the generated types. No space picker, no membership screen. The boundary is enforced in the database and the app compiles against it unchanged. **NOT VERIFIED HERE:** that the partition behaves correctly for a second account — that needs two real accounts and has not been done. |
| 10 | **Settings' three destinations** — `Account \| Integrations \| Data & Privacy`, per the 08-20 ruling above | ✅ **LIVE-VERIFIED 2026-08-30** on production, signed in as the owner. The destination bar reads exactly `Account · Integrations · Data & Privacy` and **nothing else**; `.settings-tabs` (the five-tab bar 08-11 removed) is **0 nodes**. `Data & Privacy` is one continuous page — all six sections render at once: Location, Sharing, Our apps, People and tag approvals, Your people, Messaging and event privacy, Needs attention, Your data. Integrations shows Strava & Garmin, Google Timeline and the new **Import history**. All five legacy routes redirect: `/attention`→`/settings/data/attention`, `/trash`→`/settings/data/trash`, `/export`→`/settings/data/export`, `/inbox`→`/settings/data/attention`, `/settings`→`/settings/account`. Stats sits under **Account**, as the PR said it placed it. **One thing checked and cleared rather than reported as a bug:** the heading reads "Sharing — Erica & Josh", which looks like the hardcoded household §item 9 exists to remove. It is not — `Settings.tsx:1562` builds it from `memberNames`, so it names whoever is actually in the space. Originally — PR #181. All thirteen sections of the 1,308-line page are placed under the destination whose written definition covers them and **nothing was dropped** — the mapping is in the PR body, row by row. Routes are the ones this file names: `/settings` → `/settings/account`, plus `/integrations`, `/data`, `/data/attention`, `/data/manage`, `/data/export`, `/data/trash`; `/attention`, `/trash`, `/export` and `/inbox` redirect, per "old routes redirect until links and saved URLs have migrated". `Data & Privacy` is one continuous page with all six sections rendered at once — not pills, not sub-tabs — and the live check asserts that by listing every heading. **Two things are reported rather than glossed:** (a) **Stats has no home in the approved section list.** It is not an integration and not a privacy control, and she moved it onto Settings herself and asked twice for Trips to sit inside it, so it is under **Account** as a personal summary — named here so it can be moved by instruction rather than by guess. (b) **"Messaging and event privacy" is a placeholder that says so.** There are no events and no messages — no tables, no routes — so the section renders one honest sentence rather than switches that do nothing, which is the exact defect item 6 exists to remove. Two live acceptance checks were REWRITTEN under rule 4 of `erica-asked-for.spec.ts` ("if she changes her mind the check is rewritten, with the old one noted"), and both rewrites are recorded in the file: *"Settings is ONE page, no tabs"* now asserts the three destinations by name — `.settings-tabs` stays pinned at zero, because that class was the FIVE-tab bar 08-11 removed and its CSS is deleted here — and *"Move Import and Sort Photos into Settings"* follows the two controls to `/settings/data/manage` and `/settings/integrations`, keeping every assertion. Two extras found on the way: the ledger every import writes to (`ingest_runs`, readable by members since 0202) had **never had a screen**, so Integrations now shows real import history; and `.settings-page`'s hardcoded inline `96px` bottom padding — reported by `acceptance.spec.ts` as the same inline-wrapper pattern every other route was fixed for — is `--pnav-clearance` now. |
