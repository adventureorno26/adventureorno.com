-- Build visit spans from the day things actually happened.
--
-- rebuild_place_visits decides where a visit starts and ends by collecting the
-- distinct DAYS that have evidence at a place. Those days were UTC, so an evening
-- outing landed on the next day and could split one stay into two visits, or stretch
-- one across a day it did not touch. 0143 stored the true local day; this reads it.
--
-- coalesce(local_date, ...::date) so a row whose local time could not be resolved
-- still behaves exactly as before rather than dropping out of the calculation.
