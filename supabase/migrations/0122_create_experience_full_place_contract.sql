-- 0122 — Extend create_experience()'s place contract so EVERY creation surface can
-- use it (COMPLETION-PLAN Phase 2).
--
-- Why: nine client call sites still insert into `places` directly, because the RPC
-- only accepted name/lat/lng/country/admin1/city/address/categories/saved. Those
-- callers need is_trail, bucket, needs_geocode, part_of, website and auto, and one
-- of them (the map's "drop a placeholder to name later" flow) deliberately creates
-- an UNNAMED draft. A direct insert is neither transactional with its visit nor
-- idempotent on retry, which is exactly what Phase 2 exists to remove.
--
-- What changes:
--   * p_place gains: is_trail, bucket, needs_geocode, website, auto, part_of[], review.
--   * p_place gains `allow_unnamed`. The "a new place requires a name" guard stays
--     ON by default — it is a real protection against silently creating blank
--     places — and is waived ONLY when a caller explicitly opts in for the draft
--     placeholder flow.
--   * Nothing else changes: the same INSERT means the existing BEFORE/AFTER
--     triggers (sync_place_category, sync_membership_from_part_of,
--     neutralize_junk_place, set_place_park) fire exactly as they do for a direct
--     insert, so behaviour is preserved while the write becomes atomic + idempotent.
--
-- Backwards compatible: every previously valid p_place stays valid and behaves
-- identically. New keys default to the same values the table defaults use.
