-- 0060_file_import_solo_uploader.sql — a Garmin/GPX file import is the uploader's
-- OWN activity, so attribute it to them (solo_profile = uploader) instead of the
-- "Both" default. This stops Josh's imported runs from landing in the "Both"
-- total. Genuinely shared outings still get merged by dedupe_shared_outings and
-- can be set to Both per-visit.
...
begin
  ...
  -- geom is generated from lat/lng — do NOT insert it. solo_profile = uploader.
  ...
  values
    (null, p_type, p_name, coalesce(p_distance, 0), p_moving, p_date, p_lat, p_lng,
     v_place, p_polyline, 'file', v_me, v_me)
