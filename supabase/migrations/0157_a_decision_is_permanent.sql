-- A PERSON'S DECISION IS PERMANENT. THE MACHINE MAY ONLY PROPOSE.
--
-- Erica, 2026-08-11: "all manual work I do should overwrite machine work, and
-- should not be undone by anything. We set this rule yesterday and then it was
-- ignored... Figure this out since it keeps happening."
--
-- It kept happening because the rule was enforced by REMEMBERING to set a flag,
-- and three of the writers did not:
--
--   set_visit_dates      sets manual ✓
--   set_visit_is_trip    sets manual ✓
--   create_experience    sets manual ✓
--   set_visit_solo       DOES NOT      <- "who was here" was unprotected
--   set_photo_visit      DOES NOT      <- pinning a photo was unprotected
--   ensure_visit         DOES NOT
--
-- And the loop that destroyed it: photos.visit_id is ON DELETE SET NULL, and
-- eleven functions call rebuild_place_visits — including visits_sync_photo, a
-- TRIGGER on photos. So adding a photo rebuilt the place, the rebuild deleted
-- every visit not marked manual, and the FK silently unpinned her photos. 156
-- of 176 photos have no visit for exactly this reason, and Virginia Beach grew
-- two phantom visits from photo dates.
--
-- The fix is structural, not another remembered flag:
--
--   1. A trigger marks a visit `manual` whenever a SIGNED-IN PERSON changes it.
--      auth.uid() is non-null only for a real user's request; the cron jobs and
--      edge functions run as service_role with no uid. So no future setter can
--      forget, because none of them has to remember.
--   2. Pinning a photo to a visit marks that visit decided, for the same reason.
--   3. The rebuild will not delete a visit that holds pinned photos.
--   4. A pinned photo no longer seeds a day of its own — it belongs to its
--      visit whatever its date says, which is Erica's rule verbatim.
...
comment on column public.visits.decided_at is
  'When a person last decided something about this visit. Set by the trigger below; never by a machine.';

-- 1 + 2. The two triggers that make the rule impossible to forget.
create or replace function public.visits_mark_decided()
returns trigger
language plpgsql
security definer
...
as $fn$
begin
  -- Only a signed-in person's edit counts as a decision. Machine writes run as
  -- service_role (auth.uid() is null) and leave the row rebuildable.
...
create trigger visits_mark_decided
  before update of start_date, end_date, note, solo_profile, is_trip, status
  on public.visits
  for each row execute function public.visits_mark_decided();

create or replace function public.photo_pin_marks_visit_decided()
returns trigger
language plpgsql
security definer
set search_path to 'public'
...
begin
  if new.visit_id is not null
     and new.visit_id is distinct from old.visit_id
     and auth.uid() is not null then
    update public.visits
       set manual = true, decided_at = now()
     where id = new.visit_id and not manual;
  end if;
  return new;
end $fn$;

drop trigger if exists photos_pin_marks_visit on public.photos;
create trigger photos_pin_marks_visit
  after update of visit_id on public.photos
  for each row execute function public.photo_pin_marks_visit_decided();
...
-- Backfill: protect the decisions that are already visible in the data — every
-- visit that holds a pinned photo, and every visit whose attribution a person
-- overrode. This does not invent decisions; it recognises the ones already made.
update public.visits v
   set manual = true, decided_at = coalesce(decided_at, now())
 where not v.manual
   and (exists (select 1 from public.photos ph where ph.visit_id = v.id and ph.deleted_at is null)
        or v.solo_override);
