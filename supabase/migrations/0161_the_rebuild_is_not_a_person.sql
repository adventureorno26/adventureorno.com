-- 0161 — the rebuild is not a person. Fixes a bug 0157 introduced TODAY.
--
-- 0157 added a trigger so any change a SIGNED-IN PERSON makes to a visit marks it
-- `manual`, so no automation can undo it. The discriminator was `auth.uid() is not
-- null`, reasoning that cron jobs and edge functions run as service_role with no uid.
--
-- THAT IS TRUE OF *WHO*, BUT NOT OF *WHAT*. rebuild_place_visits updates visits as
-- ordinary housekeeping, and it runs inside the signed-in person's own request —
-- eleven functions call it, including a trigger on photos. So merely uploading a photo
-- while logged in marked every visit the rebuild touched as a human decision.
--
-- Proved locally before this was written, with the 0137 test instrumented:
--     total=2  not_manual=0  manual=2
-- two derived visits, both flagged as decided by a person who decided nothing.
--
-- It is corrosive rather than dramatic: `manual` creeps across the table, the rebuild
-- becomes forbidden from correcting anything, and the visit list freezes — the exact
-- failure 0157 existed to prevent, arriving through the other door.
--
-- THE FIX: the rebuild announces itself. A TRANSACTION-LOCAL flag says "this change is
-- housekeeping" and the trigger stands down while it is set. A real edit from the app —
-- a rename, a date change, marking a trip — does not run inside the rebuild, so it
-- still marks the visit exactly as 0157 intended.
--
-- The body below is the LIVE definition read back with pg_get_functiondef, with only
-- the two set_config lines inserted, so nothing else can have changed by transcription.
--
-- ROLLBACK: recreate visits_mark_decided() from 0157 without the guard, and drop the
-- two set_config calls here.
...
as $fn$
begin
  -- Housekeeping, not a decision. Only rebuild_place_visits sets this, and only for
  -- the length of its own transaction.
  if coalesce(current_setting('aon.rebuilding', true), '') = 'on' then
    return new;
  end if;

  -- auth.uid() is non-null only inside a real person's request.
...
comment on function public.visits_mark_decided() is
  'Marks a visit as decided by a person. Stands down while rebuild_place_visits runs - the rebuild updates visits as housekeeping inside the signed-in user''s own request (0161).';
