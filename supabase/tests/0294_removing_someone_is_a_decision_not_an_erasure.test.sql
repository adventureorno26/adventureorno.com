...
--
-- WHAT 0294 ITSELF ALREADY PROVES, so that this file does not repeat it. The migration
...
--      remove. Nothing filtered these rows by status before 0294: 34 of the 41 readers
...
insert into auth.users (id, email) values
  ('aaaa0294-0000-0000-0000-000000000001','v293-ann@example.test'),
  ('bbbb0294-0000-0000-0000-000000000002','v293-bob@example.test')
  on conflict do nothing;

insert into public.profiles (id, role, display_name) values
  ('aaaa0294-0000-0000-0000-000000000001','owner','V293 Ann'),
  ('bbbb0294-0000-0000-0000-000000000002','editor','V293 Bob')
  on conflict do nothing;

insert into public.spaces (id, name, owner_profile) values
  ('11110294-0000-0000-0000-000000000001','V293 space','aaaa0294-0000-0000-0000-000000000001')
  on conflict do nothing;

delete from public.space_memberships
 where profile_id in ('aaaa0294-0000-0000-0000-000000000001','bbbb0294-0000-0000-0000-000000000002');
insert into public.space_memberships (space_id, profile_id, role) values
  ('11110294-0000-0000-0000-000000000001','aaaa0294-0000-0000-0000-000000000001','owner'),
  ('11110294-0000-0000-0000-000000000001','bbbb0294-0000-0000-0000-000000000002','editor');

insert into public.places (id, name, lat, lng, saved, space_id, created_by) values
  ('a1110294-0000-0000-0000-000000000001','V293 Overlook', 38.90, -77.30, true,
   '11110294-0000-0000-0000-000000000001','aaaa0294-0000-0000-0000-000000000001')
  on conflict do nothing;
...
insert into public.visits (id, place_id, start_date, end_date, space_id) values
  ('c1110294-0000-0000-0000-000000000001','a1110294-0000-0000-0000-000000000001',
   current_date, current_date, '11110294-0000-0000-0000-000000000001')
  on conflict do nothing;
...
update public.people set space_id = '11110294-0000-0000-0000-000000000001'
 where linked_profile in ('aaaa0294-0000-0000-0000-000000000001',
                          'bbbb0294-0000-0000-0000-000000000002');
...
select public.subject_for_visit('c1110294-0000-0000-0000-000000000001'), pe.id,
       'accepted', 'own_statement', 'user', '11110294-0000-0000-0000-000000000001'
  from public.people pe
 where pe.linked_profile = 'bbbb0294-0000-0000-0000-000000000002'
 limit 1
  on conflict do nothing;

-- The view is space-gated by 0290, so act as Ann for the reads below.
set local role authenticated;
set local request.jwt.claims = '{"sub":"aaaa0294-0000-0000-0000-000000000001","role":"authenticated"}';

do $$
declare n int;
begin
  select count(*) into n from public.visit_profiles
   where visit_id = 'c1110294-0000-0000-0000-000000000001'
     and profile_id = 'bbbb0294-0000-0000-0000-000000000002';
  if n <> 1 then
    raise exception 'SETUP: expected Bob on the visit before anything is retracted, found %', n;
  end if;
  raise notice 'PASS 0294/0 — Bob is on the visit to begin with';
end $$;
...
update public.memory_people
 ...
 where subject_id = public.subject_for_visit('c1110294-0000-0000-0000-000000000001')
   and person_id in (select id from public.people
                      where linked_profile = 'bbbb0294-0000-0000-0000-000000000002');
...
begin
  ...
  if n <> 1 then
    raise exception
      'FAIL 0294/1 — the retracted row is gone (%). Removal must leave a record: that is '
      'the whole instruction, and it is what lets an accepted tag stop being hostage to '
      'whoever made it.', n;
  end if;
  raise notice 'PASS 0294/1 — the row survives its own retraction';
end $$;
...
begin
  ...
  if n <> 0 then
    raise exception
      'FAIL 0294/2 — visit_profiles still reports a retracted participation (%). Without '
      'this predicate a removed person keeps counting in every one of the 34 readers that '
      'never look at claim_status.', n;
  end if;
  raise notice 'PASS 0294/2 — the view stops reporting a retracted participation';
end $$;
...
begin
  ...
  if n <> 1 then
    raise exception
      'FAIL 0294/3 — the view did not report Bob again after un-retracting him (%), so '
      'step 2 proved nothing: the view may be hiding him for some other reason.', n;
  end if;
  raise notice 'PASS 0294/3 — the view reports him again, so step 2 was about the status';
end $$;
