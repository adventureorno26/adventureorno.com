...
begin
  ...
  select public.occasion_count('aaaa7777-0000-0000-0000-00000000c001'::uuid) into occ_before;
  select places_count into pl_before from public.wander_stats('aaaa7777-0000-0000-0000-00000000c001'::uuid);
  ...
  select public.occasion_count('aaaa7777-0000-0000-0000-00000000c001'::uuid) into occ_after;
  ...
  select places_count into pl_after from public.wander_stats('aaaa7777-0000-0000-0000-00000000c001'::uuid);
...
begin
  ...
  select public.occasion_count('aaaa7777-0000-0000-0000-00000000c001'::uuid) into base;

  perform public.set_visit_is_trip(tv, false);
  select public.occasion_count('aaaa7777-0000-0000-0000-00000000c001'::uuid) into occ;
  if occ <> base then
...
do $$
declare p uuid; c_e int; c_j int; e_before int;
begin
  insert into public.places (name, lat, lng, saved) values ('V134 Josh Only', 45.0, -93.0, true) returning id into p;
  select public.occasion_count('aaaa7777-0000-0000-0000-00000000c001') into e_before;
  insert into public.visits (place_id, start_date, end_date, manual)
  ...
  -- default those earlier visits are Erica's alone, so her total is legitimately higher
  -- and the old shape reads a correct state as a leak. The rule is unchanged: a
  -- Josh-only visit must not move Erica's count.
  ...
  if c_e <> e_before then
    raise exception 'FAIL: a Josh-only visit changed Erica''s occasions (% -> %)', e_before, c_e; end if;
  if c_j < 1 then raise exception 'FAIL: Josh''s own visit did not count for him (%)', c_j; end if;
...
begin
  ...
  begin perform public.occasion_count('aaaa7777-0000-0000-0000-00000000c001'::uuid); exception when others then ok := true; end;
