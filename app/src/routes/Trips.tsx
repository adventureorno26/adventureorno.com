...
// each opens its normal place card with its city-grouped members.
...
export default function Trips() {
  ...
  const trips = (places ?? [])
    .filter((p) => (p.categories ?? []).includes('trip'))
    // Only trips from Dec 21, 2025 onward (undated ones still show).
    ...
    <div style={{ maxWidth: 640, margin: '20px auto', padding: '0 16px' }}>
      ...
      {places === null ? (
        ...
        <p style={{ color: 'var(--muted)' }}>
          No trips yet. Add one with <b>+ Add → Trip</b> on the map, then add the places you
          visited.
        </p>
      ) : (
